...
from database import db_connect, user_reg,viewpost1,analysis3,analysis4,analysis8,analysis9,analysis5,complaints_log,analyst_log,analysis5,analysis6,ac_add,vm1,viewcomp4,vcomp1,viewcomp1,viewcomment1,vaact,analysis1,analysis2,post_add,comment_add,adv_reg,admin_loginact,student_loginact,ins_loginact,view,send_msg,view_msg,view_std,c_search,requestact
...
def vm():
    ...
    data1 = vm1(username)
    return render_template("vm.html", p1=data1)
...
def analysis():
    ...
    data6 = analysis8()
    data7 = analysis9()
    data8 = viewpost1(username)
    return render_template("analysis.html",ana = data, ana1=data3, ana2=data4, ana3=data5, ana4=data6, ana5=data7, ana6=data8)
...
def aanalysis():
    ...
    data = analysis5()
    data3 = analysis6()
    return render_template("aanalysis.html",ana = data,ana1=data3)
...
def viewcomp():
    type = session['type']
    data = viewcomp1(type)
    return render_template("viewcomp.html",vc = data)

@app.route("/vcomp3")
def vcomp3():
    uname = request.args.get('uname')
    post = request.args.get('post')
    data = viewcomp4(uname,post)
    return render_template("reply.html",vc1 = data)
...
@app.route("/vcomp", methods = ['GET','POST'])
def vcomp():
    status = vcomp1(request.form['uname'],request.form['post'],request.form['sol'])
    if status == 1:
       return render_template("chome.html",m1="sucess")
    else:
       return render_template("chome.html",m1="failed") 
# -------------------------------Registration-----------------------------------------------------------------    
...
def acact():
   if request.method == 'POST':    
      ...
      status = ac_add(request.form['uname'],request.form['post'],request.form['type'])
...
@app.route("/complaintslog", methods=['GET', 'POST'])       
def complaintslog():
    if request.method == 'POST':
        status = complaints_log(request.form['username'], request.form['password'], request.form['type'])
        print(status)
        if status == 1:
            session['username'] = request.form['username']
            ...
            return render_template("complaints.html", m1="Login Failed")

@app.route("/analystlog", methods=['GET', 'POST'])       
def analystlog():
    if request.method == 'POST':
        status = analyst_log(request.form['username'], request.form['password'])
        print(status)
        if status == 1:
            session['username'] = request.form['username']
